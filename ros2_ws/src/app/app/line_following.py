    ...
    def mouse_callback(self, event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            if self.image_height is not None and self.image_width is not None:
                # 将调整大小后的图像坐标转换回原始图像坐标
                scale_x = self.image_width / display_size[0]
                scale_y = self.image_height / display_size[1]
                original_x = x * scale_x
                original_y = y * scale_y
                msg.data.x = original_x / self.image_width
                msg.data.y = original_y / self.image_height
