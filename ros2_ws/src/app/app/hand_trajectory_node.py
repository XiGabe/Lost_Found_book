#!/usr/bin/env python3
# encoding: utf-8
# 轨迹点发�?publish trajectory points)
import cv2
import enum
import time
import rclpy
...
def hand_angle(landmarks):
    """
    计算各个手指的弯曲角�?calculate the bending angles of each finger)
    :param landmarks: 手部关键�?hand key points)
    :return: 各个手指的角�?the angles of each finger)
    """
    angle_list = []
    # thumb 大拇�?    angle_ = vector_2d_angle(landmarks[4] - landmarks[0], landmarks[2] - landmarks[0])
    ...
    angle_list.append(angle_)
    # ring 无名�?    angle_ = vector_2d_angle(landmarks[0] - landmarks[14], landmarks[15] - landmarks[16])
    angle_list.append(angle_)
    # pink 小拇�?    angle_ = vector_2d_angle(landmarks[0] - landmarks[18], landmarks[19] - landmarks[20])
    angle_list.append(angle_)
...
def h_gesture(angle_list):
    """
    通过二维特征确定手指所摆出的手�?determine the hand gesture that the fingers are making based on the two-dimensional features)
    :param angle_list: 各个手指弯曲的角�?the bending angles of each finger)
    :return : 手势名称字符�?a string represents the name of the gesture)
    """
    thr_angle = 65.
    thr_angle_thumb = 10.
    thr_angle_s = 49.
...
class HandTrajectoryNode(Node):
    def __init__(self, name):
        ...
        self.count_miss = 0
        self.last_point = [0, 0]
        model_path = '/home/ubuntu/Lost_Found_book/ros2_ws/src/example/example/mediapipe_example/model/hand_landmarker.task'
        ...
        self.create_service(Trigger, '~/exit', self.exit_srv_callback)  # 退出玩�?exit the game)
        self.create_service(Trigger, '~/start', self.start_srv_callback)  # 进入玩法(enter the game)
        self.create_service(Trigger, '~/stop', self.stop_srv_callback)  # 退出玩�?exit the game)
    ...
    def enter_srv_callback(self, request, response):
        ...
        if self.image_sub is None:
            self.image_sub = self.create_subscription(Image, '/%s/rgb/image_raw' % self.camera, self.image_callback, 1)  # 摄像头订�?subscribe to camera)
        response.success = True
