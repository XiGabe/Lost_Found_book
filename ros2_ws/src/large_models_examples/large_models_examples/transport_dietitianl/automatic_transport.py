...
class AutomaticTransportNode(Node):
    config_path = '/home/ubuntu/Lost_Found_book/ros2_ws/src/large_models_examples/config/automatic_transport_roi.yaml'
    ...
    def __init__(self, name):
        ...
        self.selection = None  # 实时跟踪鼠标的跟踪区�?        self.track_window = None  # 要检测的物体所在区�?        self.drag_start = None  # 标记，是否开始拖动鼠�?        self.start_circle = True
    ...
    def color_detect(self, img):
        ...
        if len(contours) != 0:
            areaMaxContour, area_max = common.get_area_max_contour(contours, 10)  # 找出最大轮�?find the largest contour)
            if areaMaxContour is not None:
                if 10 < area_max:  # 有找到最大面�?the maximum area has been found)
                    ...
                    cv2.circle(img, (center_x, center_y), 5, (0, 255, 255), -1)  # 画出中心�?draw the center point)
        ...
        if not self.pick or self.debug == 'pick':
            ...
            if self.debug == 'pick':
                ...
                if self.detect_count > 10:
                    ...
                    self.debug = False
                self.get_logger().info('x_y: ' + str([object_center_x, object_center_y]))  # 打印当前物体中心的像�?print the pixel of the current object's center)
