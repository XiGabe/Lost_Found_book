...
class NavigationController(Node):
    ...
    def path_callback(self, msg):
        """处理上层发送的路径指令 Process path commands from upper layer"""
        path_length = len(msg.poses)
        
        if path_length == 0:
            self.get_logger().warn("Empty path received. Triggering emergency stop.")
            self.cancel_current_goal() # 调用取消函数 Call cancel function
            return

        self.get_logger().info(f"New path received ({path_length} points). Starting execution...")
        self.path_pub.publish(msg) # 转发路径供 Rviz 可视化 Forward path for Rviz visualization
        self.send_follow_path_goal(msg)
    
    def cancel_current_goal(self):
        """主动取消当前正在执行的 Nav2 任务 Actively cancel current Nav2 task"""
    ...
    def feedback_callback(self, feedback_msg):
        ...
        # self.get_logger().info(f'距离终点: {feedback.distance_to_goal:.2f}m', throttle_duration_sec=2.0)
        pass

    def get_result_callback(self, future):
        """任务结束（成功或失败）Task completed (success or failure)"""
    ...
    def notify_reach_status(self, success):
        """统一反馈执行状态给 Sender Unified feedback of execution status to Sender"""
        msg = Bool()
        msg.data = success
        self.reach_pub.publish(msg)
