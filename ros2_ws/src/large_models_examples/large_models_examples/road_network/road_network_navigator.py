...
class RoadNetworkNavigator(Node):
    def __init__(self):
        ...
        self.traffic_light_waypoint_id = 16    #设置交通灯识别的位置
        ...
        self.create_timer(1.0, self.publish_visualization) # 静态路网可视化
        self.create_timer(0.2, self.check_traffic_light_loop) # 红绿灯逻辑
        self.create_timer(0.1, self.timer_dynamic_viz_callback) # 动态路径剪裁
    ...
    def check_traffic_light_loop(self):
        ...
        if self.waiting_for_light:
            self.mecanum_pub.publish(Twist()) # 持续刹车
        ...
        if self.is_moving:
            ...
            if dist < check_dist_range and memory_status == 'red' and self.stop_signal_triggered:
                    ...
                    self.mecanum_pub.publish(Twist()) # 急停
    ...
    def resume_path_from_current_pose(self):
        ...
        # 这里为了性能，可以不遍历所有点，但为了安全先遍历所有
        rx, ry = robot_pos[0], robot_pos[1]
        
        # 简单的遍历寻找最近点 (如果点非常多，建议用numpy或KDTree，但几百个点循环很快)
        for i, pose_stamped in enumerate(self.current_execution_path):
            px = pose_stamped.pose.position.x
            py = pose_stamped.pose.position.y
            dist = (px - rx)**2 + (py - ry)**2
        ...
        # 稍微向前偏移索引，比如往后数 2-5 个点，让机器人有向前的趋势
    ...
    def create_dense_points(self, p1, p2):
        ...
        # 判断终点是否有自定义角度 (检查列表是否有第3个数)
        final_q = travel_q # 默认终点朝向等于行进朝向
        if len(p2) >= 3:
            custom_yaw = float(p2[2]) # 读取 YAML 里的第三个数
            final_q = self.euler_to_quaternion(custom_yaw)

        # 生成中间插值点 (全部使用行进方向 travel_q)
        for j in range(num):
            t = j / float(num)
            pose = PoseStamped()
    ...
    def reach_callback(self, msg):
        """收到 /navigation_controller/reach_goal 信号"""
        if not msg.data:
            # 1. 首先检查是不是因为红灯主动叫停的
            if self.waiting_for_light:
                self.get_logger().info(">> Goal cancelled during red light wait; pausing recovery.")
                self.is_moving = False  # 确保状态标记为停止
                return  # 把控制权交给 check_traffic_light_loop

            # 遇到障碍物失败处理：调用新的断点恢复函数
            self.get_logger().warn(">>> Goal failed/aborted; attempting [Breakpoint Recovery] ...")
            self.is_moving = False 
            self.resume_path_from_current_pose() # <--- 使用新逻辑
            return
    ...
    def publish_visualization(self):
        ...
        绘制全局路网节点和连线
        1. 绘制连线 (LINE_LIST)
        2. 绘制红色实心圆 (SPHERE_LIST)
        3. 绘制节点编号 (TEXT_VIEW_FACING)
        """
