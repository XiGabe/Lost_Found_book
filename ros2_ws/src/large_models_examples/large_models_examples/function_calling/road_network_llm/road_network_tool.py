...
tools = [
    ...
    {"type": "function",
        "function": {
            ...
            "description": "通过路网节点导航到特定功能区域，如仓库或工地等（Navigate to a specific functional area via road network nodes, such as warehouse or construction site, etc.）",
            "parameters": {
                "type": "object",
                "properties": {
                    "location": {
                        "type": "string",
                        "description": "要去的路网地点名称",
...
if os.environ.get('ASR_LANGUAGE') == 'English':
    ...
    1. Before executing any tool call, you must first give me a short, witty prompt text.
    2. After the tool call is completed, you must give me a short, witty result explanation.
    3. All text replies to me must maintain a light-hearted and fun style.
    4. If a task does not require executing a tool, you must also return a short piece of text.

    # Workflow
        1. **Task Planning**: Briefly explain your action plan (20-30 words) in a witty and humorous style.
        2. **Tool Call**: Before each tool call, you must give me a prompt (15-25 words) explaining what you are about to do.
        3. **Process Feedback**: After the tool executes, you must give me a result explanation (15-25 words), sharing progress or an interesting tidbit.
        4. **Task Summary**: Provide a witty summary upon completion (30-50 words).
        5. ** Answer in english
...
class LLMControlMove(Node):
    ...
    def initialize_variables(self):
        ...
        if self.use_depth_cam:
            ...
            if 'Pro' in self.machine_type or self.camera_type == 'usb_cam':
                self.camera = 'usb_cam'
                self.image_topic = '/%s/image' % self.camera
            else:
                self.camera = 'depth_cam'
                self.image_topic = '/%s/rgb/image_raw' % self.camera
                self.depth_topic = '/%s/depth/image_raw' % self.camera
    ...
    def setup_subs_and_pubs(self):
        ...
        if self.camera != 'usb_cam':
            self.create_subscription(Image, self.image_topic, self.image_sync_callback, 1)
        else:
    ...
    def image_sync_callback(self, ros_image):
        ...
        try:
            ...
            self.image_pair_queue.put((bgr_image,None))
        except Exception as e:
