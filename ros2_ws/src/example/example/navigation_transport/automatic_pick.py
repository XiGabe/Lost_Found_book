...
class AutomaticPickNode(Node):
    config_path = '/home/ubuntu/Lost_Found_book/ros2_ws/src/example/config/automatic_pick_roi.yaml'
    ...
    def color_detect(self, img):
        ...
        frame_mask = cv2.inRange(frame_lab, tuple(self.lab_data['lab']['Stereo'][self.target_color]['min']),
                                 tuple(self.lab_data['lab']['Stereo'][self.target_color]['max']))  # 对原图像和掩模进行位运算(perform bitwise operation to original image and mask)
        ...
        if len(contours) != 0:
            areaMaxContour, area_max = common.get_area_max_contour(contours, 10)  # 找出最大轮�?find out the contour with the maximum area)
            if areaMaxContour is not None:
                #print(111111111111, area_max)
                if 10 < area_max:  # 有找到最大面�?the maximum area is found)
                    rect = cv2.minAreaRect(areaMaxContour)  # 最小外接矩�?the minimum bounding rectangle)
                    angle = rect[2]
                    ...
                    center_x, center_y = int((ptime_start_x + pt3_x) / 2), int((ptime_start_y + pt3_y) / 2)  # 中心�?center point)
                    cv2.circle(img, (center_x, center_y), 5, (0, 255, 255), -1)  # 画出中心�?draw center point)
    ...
    def pick_handle(self, image):
        ...
        if not self.pick or self.debug:
            object_center_x, object_center_y, object_angle = self.color_detect(image)  # 获取物体颜色的中心和角度(retrieve the center and angle of the object's color)
            if self.debug:
                self.detect_count += 1
                if self.detect_count > 10:
                    ...
                    self.debug = False
                self.get_logger().info('x_y: ' + str([object_center_x, object_center_y]))  # 打印当前物体中心的像�?print the pixel coordinates of the current object's center)
                ...
                ########电机pid处理#########
                # 以图像的中心点的x，y坐标作为设定的值，以当前x，y坐标作为输入#(using the x and y coordinates of the image's center point as the set values, and the current x and y coordinates as inputs)
                ...
                if abs(self.linear_speed) == 0 and abs(self.angular_speed) == 0:
                    if self.machine_type == 'JetRover_Mecanum':
                        ...
                        if self.count_turn > 5:
                            self.count_turn = 5
                            self.status = "align"
                            if self.count_stop < 10:  # 连续10次都没在移动(hasn't moved for 10 consecutive times)
                                if object_angle < 40: # 不取45，因为如果在45时值的不稳定会导致反复移动(excludes 45, as instability in values at this angle may lead to repetitive movements)
                                    object_angle += 90
    ...
    def place_handle(self, image):
        ...
        if not self.place or self.debug:
            object_center_x, object_center_y, object_angle = self.color_detect(image)  # 获取物体颜色的中心和角度(obtain the center and the angle of the object's color)
            if self.debug:
                self.get_logger().info('x_y: %s'%str(object_center_x, object_center_y))  # 打印当前物体离中心的像素距离(print the pixel distance of the current object from the center)
            elif object_center_x > 0:
