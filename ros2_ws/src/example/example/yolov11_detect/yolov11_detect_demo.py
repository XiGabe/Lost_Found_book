...
# 支持一下默认的几种模型，更换MODEL_DEFAULT_NAME即可调用(Support the default centralized model, just change MODEL_DEFAULT_NAME to call it)
# MODEL_DEFAULT_NAME = 'yolo11s' ,'best_traffic.pt','garbage_classification'
MODEL_DEFAULT_NAME = 'garbage_classification'  # garbage_classification
...
class yoloNode(Node):
    def __init__(self, name):
        ...
        # image_topic
        image_topic = self.get_parameter('image_topic').get_parameter_value().string_value if self.has_parameter('image_topic') else '/depth_cam/rgb/image'
