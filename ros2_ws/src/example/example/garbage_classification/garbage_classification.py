...
class GarbageClassificationNode(Node):
    def __init__(self, name):
        ...
        self.create_service(Trigger, '~/start', self.start_srv_callback, callback_group=timer_cb_group)  # Enter the game(进入玩法)
        self.create_service(Trigger, '~/stop', self.stop_srv_callback, callback_group=timer_cb_group)  # Exit the game(退出玩法)
        ...
        self.start_yolov_client = self.create_client(Trigger, '/yolo/start', callback_group=timer_cb_group)
        self.start_yolov_client.wait_for_service()
        self.stop_yolov_client = self.create_client(Trigger, '/yolo/stop', callback_group=timer_cb_group)
        self.stop_yolov_client.wait_for_service()
        ...
        while self.running:
            ...
            if self.class_name is not None and not self.start_pick and not self.debug:
                ...
                if self.count > 10:
                    ...
                    self.count = 0
                    self.get_logger().info(f'Picking {self.current_class_name}')
